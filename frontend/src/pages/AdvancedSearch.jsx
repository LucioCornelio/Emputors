...
const QUICK_TAGS = [
  // Unidades
  { name: "Arbalester", type: "unit" },
  { name: "Bombard Cannon", type: "unit" },
  { name: "Hand Cannoneer", type: "unit" },
  { name: "Heavy Cavalry Archer", type: "unit" },
  { name: "Paladin", type: "unit" },
  // Tecnologías
  { name: "Bloodlines", type: "tech" },
  { name: "Bracer", type: "tech" },
  { name: "Plate Barding Armor", type: "tech" },
  { name: "Redemption", type: "tech" },
  { name: "Siege Engineers", type: "tech" },
  { name: "Thumb Ring", type: "tech" }
];
...
function AdvancedSearch() {
  ...
  const handleQuickTagClick = (tag) => {
      ...
      const dict = tag.type === 'unit' ? techtree.units : techtree.techs;
      const entry = Object.entries(dict).find(([_, val]) => val.toLowerCase() === tag.name.toLowerCase());
      
      if (entry) {
        const id = parseInt(entry[0]);
        const exactName = entry[1];
        
        setFilters(prev => {
          // Evita duplicados si ya está en la lista de filtros
          if (prev.some(f => f.name === exactName)) return prev;
          ...
          return [...prev, {
            ...
            name: exactName,
            not: false,
            operator: prev.length === 0 ? null : 'AND'
          }];
        });
      }
    };
        ...
        {/* TAG CLOUD (Quick Filters) */}
          <div style={{ marginTop: '16px', paddingTop: '16px', paddingBottom: '8px', borderTop: '1px solid #2a2d36' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {QUICK_TAGS.map(tag => {
                const icon = getIconPath(tag.name, tag.type);
                return (
                  <div 
                    key={tag.name}
                    onClick={() => handleQuickTagClick(tag)}
                    style={{ display: 'flex', alignItems: 'center', gap: '6px', backgroundColor: '#1e212b', border: `1px solid ${tag.type === 'unit' ? '#66b2ff44' : '#4caf5044'}`, padding: '4px 8px', borderRadius: '12px', cursor: 'pointer', transition: 'all 0.2s' }}
                    onMouseEnter={e => e.currentTarget.style.backgroundColor = '#2a2d36'}
                    onMouseLeave={e => e.currentTarget.style.backgroundColor = '#1e212b'}
                  >
                    {icon && <img src={icon} alt={tag.name} style={{ width: '16px', height: '16px', objectFit: 'contain' }} />}
                    <span style={{ fontSize: '11px', color: tag.type === 'unit' ? '#66b2ff' : '#4caf50', fontWeight: '500' }}>{tag.name}</span>
                  </div>
                );
              })}
            </div>
